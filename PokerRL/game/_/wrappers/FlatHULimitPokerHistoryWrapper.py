...
class FlatHULimitPokerHistoryWrapper(Wrapper):
    ...
    def get_current_obs(self, env_obs=None, multi_tensor_obs=False, observer_seat=None):
        if multi_tensor_obs is True and env_obs is None:
            return self.env.get_current_obs(is_terminal=False, multi_tensor_obs=True, observer_seat=observer_seat)
        if multi_tensor_obs is True and env_obs:
            return env_obs
        if env_obs is None:
            return np.concatenate(
                (
                    self.env.get_current_obs(is_terminal=False, observer_seat=observer_seat),
    ...
    def set_to_public_tree_node_state(self, node):
        ...
        self.reset(multi_tensor_obs=node.tree._multi_tensor_obs)
        self._reset_state()
        for sd in state_seq:
