...
class Wrapper:
    ...
    def _return_obs(
        self, rew_for_all_players, done, info, env_obs=None, multi_tensor_obs=False
    ):
        return (
            self.get_current_obs(env_obs=env_obs, multi_tensor_obs=multi_tensor_obs),
            rew_for_all_players,
            done,
            info,
        )
    ...
    def step(self, action, multi_tensor_obs=False):
        ...
        """
        env_obs, rew_for_all_players, done, info = self.env.step(
            action, multi_tensor_obs=multi_tensor_obs
        )
        ...
        return self._return_obs(
            ...
            info=info,
            multi_tensor_obs=multi_tensor_obs,
        )
    ...
    def step_from_processed_tuple(self, action):
        ...
        Returns:
            obs, reward, done, info
        """
        env_obs, rew_for_all_players, done, info = self.env.step_from_processed_tuple(
            action
        )
        self._pushback(env_obs)
        return self._return_obs(
    ...
    def step_raise_pot_frac(self, pot_frac):
        ...
        Returns:
            obs, reward, done, info
        """
        env_obs, rew_for_all_players, done, info = self.env.step_raise_pot_frac(
            pot_frac=pot_frac
        )
        self._pushback(env_obs)
        return self._return_obs(
    ...
    def reset(self, deck_state_dict=None, multi_tensor_obs=False):
        env_obs, rew_for_all_players, done, info = self.env.reset(
            deck_state_dict=deck_state_dict, multi_tensor_obs=multi_tensor_obs
        )
        self._reset_state()
        self._pushback(env_obs)
        return self._return_obs(
    ...
    def get_current_obs(self, env_obs=None, observer_seat=None):
        raise NotImplementedError
