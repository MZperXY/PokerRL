...
class PokerEnv:
    ...
    def __init__(
        ...
        self.observation_space, self.obs_idx_dict, self.obs_parts_idxs_dict = (
        ...
        # Action history tracking for embedding-based observations
        # Dict mapping round -> list of (player_seat_id, action_type) tuples
        # action_type: 0=FOLD, 1=CHECK_CALL, 2=BET_RAISE
    ...
    def _construct_obs_space(self):
        ...
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            ...
            # NEW: Observer's seat (so agent knows its own identity)
            # This tells the receiving agent which seat it is
            for i in range(
                self.N_SEATS
            ):  # ....................................................... observer_seat onehot
                _table_space.append(
                    get_discrete(1, "observer_seat_" + str(i), next_idx)
                )
    ...
    def _deal_turn(self):
        self.board[self.N_FLOP_CARDS : self.N_FLOP_CARDS + self.N_TURN_CARDS] = (
    ...
    def _deal_next_round(self):
        ...
        if self.current_round == Poker.PREFLOP:
            ...
            raise ValueError(self.current_round)
        self._update_card_multi_tensor_board()
    ...
    def _step(self, processed_action, multi_tensor_obs: bool = False):
        """
        ...
        # Track action in history for embedding-based observations
        # Format: (player_seat_id, action_type) where action_type is 0=FOLD, 1=CHECK_CALL, 2=BET_RAISE
        if self.action_history is not None:
            self.action_history[self.current_round].append(
                (self.current_player.seat_id, processed_action[0])
            )
        ...
        if multi_tensor_obs:
            previous_action_tensor = np.zeros(self._args.N_ACTIONS, dtype=np.int8)
            previous_action_tensor[self.last_action[0]] = (
                1 if self.actions_in_round > 0 else 0
            )
            ...
            last_action_tensor = np.zeros(self._args.N_ACTIONS, dtype=np.int8)
            last_action_tensor[self.last_action[0]] = 1
            self.actions_in_round += 1
            self._update_card_multi_tensor_actions(
                previous_action_tensor, last_action_tensor, legal_actions
            )
            card = self.multi_tensor_observation["card"]
            action = self.multi_tensor_observation["action"]
            assert np.sum(card[0]) <= 1
            if last_action_tensor[0] == 2:
                clipped_legal_actions = np.clip(legal_actions, 0, 1)
                assert np.sum(clipped_legal_actions) > 1
        else:
            # Keep last_action updated in simple-observation mode so wrappers
            # (e.g. FlatHULimitPokerHistoryWrapper) can build action history.
            self.last_action = [
                processed_action[0],
                processed_action[1],
                self.current_player.seat_id,
        ...
        return self._get_current_step_returns(
            is_terminal=is_terminal, info=info, multi_tensor_obs=multi_tensor_obs
        )

    def _create_infos(self) -> dict:
        """
        This function is called at the end of each step to edit the infos dict."""

        state_dict = self.state_dict()
        main_pot = state_dict["main_pot"]
        seat_0 = state_dict["seats"][0]
        assert seat_0["seat_id"] == 0
        seat_1 = state_dict["seats"][1]
        assert seat_1["seat_id"] == 1
        player_0_bet = seat_0["current_bet"]
        player_1_bet = seat_1["current_bet"]
        player_0_hand = seat_0["hand"]
        player_1_hand = seat_1["hand"]
        current_player = state_dict["current_player"]
        player_0_total = main_pot / 2 + player_0_bet
        player_1_total = main_pot / 2 + player_1_bet
        min_to_lose = player_0_total if current_player == 0 else player_1_total
        max_to_win = player_1_total if current_player == 0 else player_0_total
        n_actions_episode = state_dict["n_actions_this_episode"]

        # Normalize clipping values using REWARD_SCALAR (to align with batch["rewards"])
        min_to_lose = min_to_lose / self.REWARD_SCALAR
        max_to_win = max_to_win / self.REWARD_SCALAR
        ...
        infos = {
            ...
            "player_0_total": player_0_total,  # These remain in raw chip values
            "player_1_total": player_1_total,  # These remain in raw chip values
            "n_actions_episode": n_actions_episode,
            "clipping": [min_to_lose, max_to_win],
        }
        return infos
    ...
    def _get_current_step_returns(self, is_terminal, info, multi_tensor_obs: bool):
        if multi_tensor_obs is False:
            obs = self.get_current_obs(is_terminal)
        else:
            obs = self.multi_tensor_observation
    ...
    def _get_table_state(self, normalization_sum, observer_seat=None):
        """
        Generate the table state portion of the observation.

        Args:
            normalization_sum: Value to normalize chip amounts by (avg starting stack)
            observer_seat: Optional seat ID of the agent receiving this observation.
                          If None, defaults to current_player.seat_id (the acting player).
                          This field helps the agent know its own identity.
        """
        # Default observer to current player (who is about to act)
        if observer_seat is None:
            observer_seat = self.current_player.seat_id
        ...
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            ...
            # NEW: observer's seat (agent's own identity)
            # This one-hot tells the agent which seat it is
            x = [0] * self.N_SEATS
            x[observer_seat] = 1
            community_state += x
    ...
    def reset(self, deck_state_dict=None, multi_tensor_obs: bool = False):
        """
        ...
        self.multi_tensor_observation = self._reset_multi_tensor_observation()

        # Reset action history - dict mapping round index to list of (player_id, action_type) tuples
        self.action_history = {r: [] for r in self.ALL_ROUNDS_LIST}
        # players
        for p in self.seats:
            p.reset()
        ...
        info = self._create_infos()
        return self._get_current_step_returns(
            is_terminal=False, info=info, multi_tensor_obs=multi_tensor_obs
        )
    ...
    def step(self, action, multi_tensor_obs=False):
        """
        Args:
        ...
        return self._step(
            processed_action=processed_action, multi_tensor_obs=multi_tensor_obs
        )
    ...
    def state_dict(self):
        env_state_dict = {
            ...
            EnvDictIdxs.actions_in_round: copy.deepcopy(self.actions_in_round),
            EnvDictIdxs.multi_tensor_observation: copy.deepcopy(
                self.multi_tensor_observation
            ),
            ...
            EnvDictIdxs.last_raiser: (
                None if self.last_raiser is None else self.last_raiser.seat_id
            ),
            # idx in _env.seats
        ...
        # Add action history for embedding-based observations
        env_state_dict[EnvDictIdxs.action_history] = copy.deepcopy(self.action_history)
    ...
    def load_state_dict(self, env_state_dict, blank_private_info=False):
        ...
        self.actions_in_round = copy.deepcopy(
            env_state_dict[EnvDictIdxs.actions_in_round]
        )
        self.multi_tensor_observation = copy.deepcopy(
            env_state_dict[EnvDictIdxs.multi_tensor_observation]
        )
        ...
        # Load action history for embedding-based observations
        if EnvDictIdxs.action_history in env_state_dict:
            self.action_history = copy.deepcopy(
                env_state_dict[EnvDictIdxs.action_history]
            )
        else:
            # Backwards compatibility: initialize empty if not present
            self.action_history = {r: [] for r in self.ALL_ROUNDS_LIST}
    ...
    def get_current_obs(self, is_terminal, multi_tensor_obs=False, observer_seat=None):
        """
        ...
        Args:
            ...
            multi_tensor_obs (bool): Whether to return multi-tensor observation format
            observer_seat (int, optional): The seat ID of the agent receiving this observation.
                                          If None, defaults to current_player.seat_id.
                                          This allows the wrapper to specify which agent is
                                          receiving the observation, enabling the agent to
        ...
        if is_terminal:
            return np.zeros(
                shape=self.observation_space._shape, dtype=np.float32
            )  # terminal is all zero
        if multi_tensor_obs is True:
            return self.get_current_obs_multi_tensor()
        normalization_sum = (
            float(sum([s.starting_stack_this_episode for s in self.seats]))
            / self.N_SEATS
        )
        return np.array(
            self._get_table_state(
                normalization_sum=normalization_sum, observer_seat=observer_seat
            )
    ...
    def get_current_obs_multi_tensor(self):
        """
        This function can be useful for manually setting the environment to a desired state and then getting the
        formatted observation from it without actually stepping it.
        ...
        """
        if getattr(self, "multi_tensor_observation") is False:
            raise ValueError("Multi tensor obs should be created first!")
        return self.multi_tensor_observation
    ...
    def _get_card_multi_tensor_shape(self):
        size_of_streets = len(self.ALL_ROUNDS_LIST)
        return (size_of_streets + 3, self.N_SUITS, self.N_RANKS)

    def _get_action_tensor_shape(self):
        action_tensor_shape = (
            len(self.ALL_ROUNDS_LIST) * (self.MAX_N_RAISES_PER_ROUND[Poker.PREFLOP] + 2)
            + 2,
            4,
            9,
        )
        return action_tensor_shape

    def _reset_multi_tensor_observation(self):
        action_tensor_shape = self._get_action_tensor_shape()
        card_multi_tensor_shape = self._get_card_multi_tensor_shape()
        action_tensor = np.zeros(action_tensor_shape, dtype=np.float32)
        card_multi_tensor = np.zeros(card_multi_tensor_shape, dtype=np.float32)
        multi_tensor_observation = {
            "card": card_multi_tensor,
            "action": action_tensor,
        }
        self.actions_in_round = 0
        return multi_tensor_observation

    def _update_card_multi_tensor_board(self):
        """
        Update the card tensor with board cards.

        Card tensor layout: (n_streets + 3, n_suits, n_ranks)
        - Position 0: Reserved for player's hole card (added by wrapper)
        - Positions 1 to n_flop: Flop cards
        - Position -4: Turn card
        - Position -3: River card
        - Positions -2, -1: Auxiliary/summed cards

        Board cards are stored as [[rank, suit], [rank, suit], ...]
        Tensor is indexed as [plane, suit, rank]

        BUG FIX: Previously TURN and RIVER incorrectly unpacked as (suit, rank).
        Now all streets correctly unpack as (rank, suit).
        """
        card_multi_tensor = self.multi_tensor_observation["card"]
        current_round = self.current_round

        if current_round == Poker.FLOP:
            for i, (rank, suit) in enumerate(self.board[: self.N_FLOP_CARDS]):
                card_multi_tensor[current_round + i, suit, rank] = 1

        elif current_round == Poker.TURN:
            # FIX: Changed from (suit, rank) to (rank, suit) to match board storage
            ...
            # FIX: Changed from (suit, rank) to (rank, suit) to match board storage
            for rank, suit in self.board[self.N_FLOP_CARDS + self.N_TURN_CARDS :]:
                card_multi_tensor[len(card_multi_tensor) - 3, suit, rank] = 1

        self.multi_tensor_observation["card"] = card_multi_tensor

    def _update_card_multi_tensor_actions(
        self, previous_action, last_action, legal_actions
    ):
        """
        Update the action tensor with the latest action information.

        Action tensor layout: (n_action_slots, 4, 9)
        - Dimension 0: Action slot index (round * actions_per_round + action_idx)
        - Dimension 1: [row_0=seat_0_actions, row_1=seat_1_actions, row_2=sum, row_3=legal_mask]
        - Dimension 2: Action one-hot encoding (fold, call, raise, ...)

        BUG FIX: Previously agent_idx was based on SB_POS comparison which caused
        inconsistent encoding (agent/opponent swapped depending on who acts).
        Now we use a fixed convention: row 0 = seat 0, row 1 = seat 1.
        This is consistent and allows the agent to understand action history.

        The current player's seat_id indicates WHO just acted.
        """
        if previous_action[0] is None:
            previous_action = np.zeros(len(last_action), dtype=np.float32)

        action_multi_tensor = self.multi_tensor_observation["action"]
        current_round = self.current_round
        actions_in_round = self.actions_in_round

        # FIX: Use consistent indexing based on seat_id, not SB position
        # Row 0 always represents seat 0's actions
        # Row 1 always represents seat 1's actions
        # This way the agent can always know which player took which action
        current_seat = self.current_player.seat_id
        opponent_seat = 1 - current_seat

        possible_actions_in_a_round = self.MAX_N_RAISES_PER_ROUND[Poker.PREFLOP] + 3
        current_tensor = action_multi_tensor[
            current_round * possible_actions_in_a_round + actions_in_round
        ]

        # Store actions by seat_id for consistent encoding
        # last_action is from current_player (who just acted)
        # previous_action is from the other player
        current_tensor[current_seat, : len(last_action)] = last_action
        current_tensor[opponent_seat, : len(last_action)] = previous_action

        # Row 2: Sum of all actions this slot (for convenience)
        sum_of_actions = np.clip(
            np.array(last_action) + np.array(previous_action), 0, 1
        )
        current_tensor[2, : len(last_action)] = sum_of_actions

        # Row 3: Legal actions mask (populated by wrapper)
