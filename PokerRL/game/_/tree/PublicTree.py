...
class PublicTree:
    ...
    def __init__(
        ...
        self._multi_tensor_obs = multi_tensor_obs

        # set up optional graphical tree export
        # _dir_tree_vis = os.path.join("C:\\" if os.name == 'nt' else os.path.expanduser('~/'), "PokerRL_Viz")
        _dir_tree_vis = "/Users/varghaferenc/Documents/ReinforcementLearning/Poker/poker_framework/AlphaHoldem/Viz"
        ...
        self._env.reset(multi_tensor_obs=self._multi_tensor_obs)
    ...
    def _get_children_of_action_node(self, parent):
        ...
        if parent.p_id_acting_next == self.CHANCE_ID:
            ...
            for action in parent.allowed_actions:
                self._env.load_state_dict(parent.env_state)
                _, __, is_terminal, info = self._env.step(action, multi_tensor_obs=True)
